    ...
    <div class="mx-auto max-w-6xl px-4 py-14 sm:px-6 lg:px-8">
        ...
        <div class="mt-12 flex flex-col gap-4 border-t border-border pt-8 text-sm text-muted sm:flex-row sm:items-center sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-xs text-faint">{{ config('brand.company.address.street') }}, {{ config('brand.company.address.locality') }}, {{ config('brand.company.address.region') }}, {{ config('brand.company.address.postcode') }}.</p>
                <p class="mt-1 text-xs text-faint">Registered in England and Wales. Company No. {{ config('brand.company.company_number') }}. VAT No. {{ config('brand.company.vat_number') }}. ICO No. {{ config('brand.company.ico_number') }}. Built to WCAG 2.2 AA.</p>
            </div>
